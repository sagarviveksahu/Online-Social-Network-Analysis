...
def add_all_friends(twitter, users):
    ...
    for i in range(len(screen_names)):
        users[i]['friends'] = get_friends(twitter, screen_names[i])
...
def print_num_friends(users):
    ...
    screen_names = [u['screen_name'] for u in users]
    for i in range(len(screen_names)):
        print('%s %d' %(screen_names[i], len(users[i]['friends'])))
...
def friend_overlap(users):
    ...
    for i in range(len(screen_names)-1):
        for j in range(i+1,len(screen_names)):
            ...
            l.append(tuple([screen_names[i], screen_names[j],s]))
...
def followed_by_hillary_and_donald(users, twitter):
    ...
    screen_names = [u['screen_name'] for u in users]
    if 'HillaryClinton' or 'realDonaldTrump' in screen_names:
        user_id = set(users[screen_names.index('HillaryClinton')]['friends']) & set(users[screen_names.index('realDonaldTrump')]['friends'])
        resource = 'users/lookup'
        params = {'user_id':user_id.pop()}
        request = robust_request(twitter, resource, params, max_tries=5)
        for r in request:
...
def create_graph(users, friend_counts):
    ...
    new_friend_counts = Counter({j: k for j, k in friend_counts.items() if k > 1})
    graph.add_nodes_from(new_friend_counts.keys())
    for i in new_friend_counts.keys():
        for j in range(len(screen_names)):
            if i in users[j]['friends']:
                graph.add_edge(screen_names[j],i)
    return graph
