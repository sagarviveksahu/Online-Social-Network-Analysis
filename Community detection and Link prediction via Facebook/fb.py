...
def bfs(graph, root, max_depth):
    ...
    while len(d) > 0:
        u = d.popleft()
        seen.add(u)
        level = node2distances[u]
        if level >= max_depth:
            break
        for v in graph.neighbors(u):
            if v not in seen:
                seen.add(v)
                node2distances[v] = level + 1
                node2parents[v].append(u)
                d.append(v)
                node2num_paths[v] = len(node2parents[v])
            elif node2distances[v] == node2distances[u] + 1 and u not in node2parents[v]:
                node2parents[v].append(u)
                node2num_paths[v] += node2num_paths[u]
...
def approximate_betweenness(graph, max_depth):
    ...
    for i in graph.nodes():
        ...
        xx = bottom_up(i, node2distances, node2num_paths, node2parents)
        for i,j in xx.items():
            if i not in lol:
                lol[i] = j/2
            else:
...
def partition_girvan_newman(graph, max_depth):
    ...
    lol = approximate_betweenness(g, max_depth)
    rev_lol = sorted(lol.items(), key=lambda x: (-x[1],x[0]))
    while len(components) == 1:
        g.remove_edge(*(rev_lol[i][0]))
        i += 1
        components = [c for c in nx.connected_component_subgraphs(g)]
    return components
    pass
...
def cut(S, T, graph):
    ...
    cut = 0
    for ed in graph.edges():
        if ed[0] in S and ed[1] in T:
            cut = cut + 1
        if ed[1] in S and ed[0] in T:
            cut = cut + 1
    return cut
    pass
...
def norm_cut(S, T, graph):
    ...
    """
    ###TODO
    norm = (cut(S, T, graph)/volume(S,graph)) + (cut(S,T,graph)/volume(T,graph))
    return norm
    pass
...
def score_max_depths(graph, max_depths):
    ...
    score = []
    for yoyo in max_depths:
        partitions = partition_girvan_newman(graph, yoyo)
        ax = norm_cut(partitions[0], partitions[1], new_graph)
        score.append((yoyo,ax))
    return score
    pass
...
def make_training_graph(graph, test_node, n):
    ...
    for i in range(n):
        g.remove_edge(nbrs[i],test_node)
    return g
    pass
...
def jaccard(graph, node, k):
    ...
    for n in graph.nodes():
        if n not in neighbors and n != node:
            new_neighbors = set(graph.neighbors(n))
            scores.append(((node,n), len(neighbors & new_neighbors) / len(neighbors | new_neighbors)))
...
def evaluate(predicted_edges, graph):
    ...
    for i in predicted_edges:
        if i in graph.edges() or i[::-1] in graph.edges():
            c += 1
    return c/len(predicted_edges)
    pass
