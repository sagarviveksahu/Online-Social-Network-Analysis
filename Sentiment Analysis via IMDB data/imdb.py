...
def vectorize(tokens_list, feature_fns, min_freq, vocab=None):
    ...
    for i in tokens_list:
        l.append(featurize(i,feature_fns))
        for j in featurize(i,feature_fns):
            dic.update({j[0] : dic[j[0]] + 1})
            if j[0] not in temp_vocab:
                temp_vocab.update({j[0] : len(temp_vocab)})
    if vocab == None:
        vocab = {}
        for i in sorted(temp_vocab):
            if i in dic:
                if dic[i] >= min_freq:
                    if i not in vocab:
                        vocab.update({i : len(vocab)})
    for i in l:
        for j in i:
            if j[0] in vocab:
                index = vocab[j[0]]
...
def cross_validation_accuracy(clf, X, labels, k):
    ...
    for train, test in kfold:
        ...
        a.append(accuracy_score(labels[test], clf.predict(X[test])))
        mean = np.mean(a)
    return mean
...
def eval_all_combinations(docs, labels, punct_vals,
    ...
    for i in range(len(feature_fns)):
        for j in combinations(feature_fns, i + 1):
            feat.append(j)
    for p in punct_vals:
        tokens = [tokenize(d, p) for d in docs]
        for f in feat:
            for m in min_freqs:
                dic = {}
                X, vocab = vectorize(tokens, f, m)
                cv = cross_validation_accuracy(LogisticRegression(), X, labels, 5)
                dic.update({'punct' : p})
                dic.update({'features' : f})
                dic.update({'min_freq' : m})
                dic.update({'accuracy' : cv})
                l.append(dic)
    l = sorted(l, key=lambda x: (-x['accuracy'], -x['min_freq']))
    return l
...
def mean_accuracy_per_setting(results):
    ...
    l = []
    for i in results:
        comb.append(("min_freq", i['min_freq']))
        comb.append(("punct", i['punct']))
        comb.append(("features", i['features']))
    for j in set(comb):
        acc_res = [k['accuracy'] for k in results if k[j[0]] == j[1]]
        mean = np.mean(acc_res)
        if 'features' in j[0]:
            fil = []
            for f in j[1]:
                fil.append(f.__name__)
            feat = str(("features=" + ' '.join(fil)))
            l.append((mean, feat))
        else:
            feat = (j[0] + "=" + str(j[1]))
            l.append((mean, feat))
...
def print_top_misclassified(test_docs, test_labels, X_test, clf, n):
    ...
    for i in range(len(test_docs)):
        ...
        if pred[i]!=test_labels[i]:
            test['doc'] = test_docs[i]
            test['truth'] = test_labels[i]
            test['predicted'] = pred[i]
            test['proba'] = prob[i][pred[i]]
            l.append(test)
    for j in sorted(l,key=lambda x: -x['proba'])[:n]:
        print('\n truth=%d predicted=%d proba=%.6f \n %s' %(j['truth'],j['predicted'],j['proba'],j['doc'] ))
